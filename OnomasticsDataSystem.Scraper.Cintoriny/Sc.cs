...
namespace OnomasticsDataSystem.Scraper.Cintoriny
{
	public class Sc
	{
		...
		public Sc()
		{
			...
			_client.DefaultRequestHeaders.Add("User-Agent", "Mozilla/5.0 (scrapyghter/1.1)");
		}
		...
		public async Task RunAsync()
		{
			...
			for (int cidIndex = 0; cidIndex < allCids.Count; cidIndex++)
			{
				...
				var cityDict = await GetCityNamesAsync(
					"https://www.cintoriny.sk/api/getMapInfo.php",
					c
				);

				//Nájdeme identifikátory
				var identifiers = await FindIdentifiers(
					"https://www.cintoriny.sk/api/getMap.php",
					c
				);
				//zapis do suboru kontrola
				//string outputPath = "identifiers.txt";
				//await File.WriteAllLinesAsync(outputPath, identifiers);
				...
				// Počkaj kým sa spracujú všetky ID pre tento CID
				await Task.WhenAll(tasks);

				// 4️⃣ Zapíš CSV pre tento cintorín
			...
			while (attempts < 3)
			{
				...
				new KeyValuePair<string,string>("cId", cidValue)
					...
					foreach (Match m in Regex.Matches(body, @"hrob=([^\r\n&]+)", RegexOptions.IgnoreCase))
					{
						...
						// nový typ: "1..123" -> "1____123"
						else if (Regex.IsMatch(ident, @"^\d+\.\.+\d+$"))
						{
							Console.WriteLine($"som tu1");
							// najprv nájdeme čísla zo surového identu
						...
						//1.5) Formát: 1.8.1,2 (bodky + viac čísel oddelených čiarkou)
						else if (ident.Contains(".") && ident.Contains(",") && !ident.Contains(" "))
						{
							Console.WriteLine($"som tu2");
							//Console.WriteLine($"sp, tu");
							// Rozdelenie na ľavú časť (prefix) a posledný blok s čiarkami
							var lastDot = ident.LastIndexOf('.');
							var prefix = ident.Substring(0, lastDot + 1);  // vrátane bodky
							var suffix = ident.Substring(lastDot + 1);      // napr: "1,2"
							...
							foreach (var n in nums)
							{
								...
								full = full.Replace(".", "__");                    // odstráni bodky
								full = Regex.Replace(full, @"\s+", "__");      // nahradí medzery

								identifiers.Add(full);
							}
							continue;
						}
						// 2) formát typu "11.  .171.  .a"
						...
						// 2.5) Formát ABC.12.X – všetky časti môžu obsahovať čísla aj písmená
				...
				catch (HttpRequestException ex)
				{
					...
					if (is500)
					{
						Console.WriteLine($"[CID {cidValue}] ❗ anoServer Error 500 (pokus {attempts}/3)");
					}
					else
					{
		...
		public async Task<Dictionary<string, string>> GetCityNamesAsync(string infoUrl, string cid)
		{
			...
			while (attempts < 3)
			{
				try
				{
					...
					// Inicializácia session – len GET na stránku cintorína
		...
		{
			var foundIds = new HashSet<string>();//zoznam cld

			using var playwright = await Playwright.CreateAsync();//inicializacia
			...
			// chvíľu čakaj, aby prišli všetky odpovede
			await page.WaitForTimeoutAsync(3000);

			await browser.CloseAsync();

			// vráť usporiadaný zoznam
		...
		private static List<Person> ExtractNames(string html)
		{
			...
			foreach (Match m in rowRegex.Matches(html))
			{
				...
				// --- 1️⃣ FullNameRaw (celé meno bez zásahov) ---
				...
				// --- 2️⃣ Pôvodné spracovanie mena (čistenie + delenie) ---
